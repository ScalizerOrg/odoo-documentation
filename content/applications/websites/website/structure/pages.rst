...
can be adapted:
 ...
 - :guilabel:`Has Parent Page`: Toggle the switch on to add a parent page in breadcrumbs format.
 - :guilabel:`Published`: Enable it to publish the page.
 - :guilabel:`Publish on`: To publish the page at a specific date and time, click the field,
...
To duplicate one or several pages, go to :menuselection:`Website --> Site --> Pages`, select the
relevant page(s), click :icon:`fa-cog` :guilabel:`Actions`, and select :icon:`fa-files-o`
:guilabel:`Duplicate`. In the :guilabel:`Confirmation` window, enter a :guilabel:`Page Name`, then
click :guilabel:`Ok`. By default, the new page is not published; it is added after the originally
duplicated page in the menu. Use the :doc:`menu editor <header_footer>` to remove it from the menu
...
To delete delete one or several pages, proceed as follows:

#. Go to :menuselection:`Website --> Site --> Pages`, and select the relevant page(s).
#. Click :icon:`fa-cog` :guilabel:`Actions`, and select :icon:`fa-trash` :guilabel:`Delete`.
...
To access existing URL redirections and create new ones, go to :menuselection:`Website -->
Configuration --> Redirects`, click :guilabel:`New` in the :guilabel:`Rewrite` view, then adapt the
fields:
...
.. tip::
   You can also define the order in which redirections are performed, e.g., in the case
   of redirect chains (i.e., a series of redirects where one URL is redirected to another one, which
   is itself further redirected to another URL). To do so, :doc:`activate the developer mode
   </applications/general/developer_mode>`, access the :guilabel:`Rewrite` form, and define a
   :guilabel:`Sequence`.
