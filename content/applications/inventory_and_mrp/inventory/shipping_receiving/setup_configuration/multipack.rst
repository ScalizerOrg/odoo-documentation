...
In some cases, a delivery order with multiple items may need to be shipped in multiple packages.
This may be necessary if the items are too large to ship in a single package, or if certain items
cannot be packaged together. Shipping a single delivery order in multiple packages allows each item
to be packaged differently, without creating multiple delivery orders.
...
To split items in the same delivery order across multiple packages, navigate to the
:menuselection:`Inventory` app. On the *Inventory Overview* page, click the :guilabel:`# to Process`
button on the :guilabel:`Delivery Orders` card. On the *Delivery Orders* page, select a delivery
order that has multiple items, multiple quantities of the same item, or both.

On the *Operations* tab, select the :icon:`fa-list` :guilabel:`(Open Move)` icon for the product
...
This opens the *Open: Stock move* pop-up window. In the table in the pop-up window, the
:guilabel:`Quantity` column shows the total quantity of the product included in the delivery order.

If the full quantity will be shipped in the first package, enter the :guilabel:`Demand` value in the
:guilabel:`Quantity` column. If less than the full quantity will be shipped in the first package,
enter a smaller number than the :guilabel:`Demand` quantity. Then, specify a :guilabel:`Destination
...
If packaging multiple products into the same package, repeat the steps for each product. Be sure to
specify the same :guilabel:`Destination Package` in the *Open: Stock move* window.
...
If some items will be shipped later than others, there is no need to package them until they are
ready to ship. Instead, create a back order for the items being shipped later.
...
Change the :guilabel:`Quantity` for the back ordered items to `0` or to the partial quantity being
shipped. Then, click the :guilabel:`Validate` button, and a :guilabel:`Create Backorder?` pop-up
window appears. Click the :guilabel:`Create Backorder` button. Doing so immediately confirms the
...
.. image:: multipack/backorder-pop-up.png
   :alt: Create a back order.

The back order delivery order will be listed in the chatter of the original delivery order, with a
message that reads :guilabel:`The backorder WH/OUT/XXXXX has been created`. Click on
:guilabel:`WH/OUT/XXXXX` in the message to view the back order delivery order.

.. image:: multipack/backorder-chatter.png
   :alt: The back order delivery order listed in the chatter of the original delivery order.

The back order delivery order can also be accessed by navigating to :menuselection:`Inventory`,
...
.. image:: multipack/back-orders-button.png
   :alt: The Back Orders link on the Delivery Orders card.

When the remaining items are ready to be shipped, navigate to the back order delivery order. The
items can be shipped in a single package by clicking :guilabel:`Validate`, or shipped in multiple
packages by following the steps outlined in :ref:`inventory/shipping/multiple-packages`.

It is also possible to ship some of the items while creating a new back order for the rest. To do
so, follow the same steps used to create the first back order.
