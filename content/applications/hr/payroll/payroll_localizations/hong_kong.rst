...
:menuselection:`Settings app --> Companies --> Manage Companies`. Create a new company or
select an existing one, and configure the following fields:
...
company's information in the :guilabel:`Hong Kong Localization` section:
...
- :guilabel:`Configure your MPF Schemes`: Click this to configure MPF schemes (Manulife,
  AIA, etc.) for employee benefits.
...
Navigate to :menuselection:`Payroll app --> Configuration --> Work Entries --> Working
Schedules` to check or configure working schedules.

In configuring a working schedule, employers can define:

- :guilabel:`Public Holidays`: Specific holidays that are used for the working schedule.
- :guilabel:`Day of Week`, :guilabel:`Day Period`, :guilabel:`Work From`, :guilabel:`Work
  To`, :guilabel:`Duration`, and :guilabel:`Work Entry Type`.

.. image:: hong_kong/working-schedules.png
   :alt: The HK Standard 40 hours/week working schedule setup.

.. note::
   - Odoo defines only one dynamic holidays option. Employers can define desired public
     holidays manually. Click :guilabel:`New` to define public holidays: :guilabel:`Name`,
     :guilabel:`Start Date`, :guilabel:`End Date`, :guilabel:`Working Hours`, and
     :guilabel:`Work Entry Type`.
   - In order to delete public holidays, select the record and
     click the (gear) icon :menuselection:`Actions --> Delete`. For more information, refer to
     the :ref:`public holidays <time_off/public-holidays>` documentation.
...
.. _payroll/hk/contract_management:
...
To create a new employee contract in the **Payroll** app:
Go to :menuselection:`Payroll app --> Employees --> Employees`. Navigate to an employee
record and select the :guilabel:`Payroll` tab. Assign an end date to the current contract.
Click the :guilabel:`New Contract` button and select the new contract date. A new employee
version is created according to the new contract starting date.

.. warning::
   Ensure you end the current contract before creating a new contract. Only one running
   contract for each employee is allowed. Contracts cannot be nested as sub-records.

Version creation
----------------

When a new employee contract is created, an employee version is recorded automatically, and
the version date is stored. Employee versions keep a history of changes to an employee's
information, including data relevant to payroll. This allows HR to review and track an
employee's details over time.

To create a new employee version manually:
Go to the employee record, click the :guilabel:`+ (Create new version)` icon, and select the
version date. Update the employee information as needed, then save.

Employee records
----------------

Employee versions can be reviewed under :menuselection:`Payroll app --> Employees -->
Employee Records` (requires developer mode activated). To delete an employee version, select
the record, click the :guilabel:`Actions` button, and select :guilabel:`Delete`. An employee
must always have at least one active version.

.. image:: hong_kong/employee-records.png
   :alt: The Employee Records list tracking historical versions and contracts.
...
In the **Payroll** app dashboard, employees without MPF registration, without bank account
numbers, or without identification numbers appear under :guilabel:`Warnings`. Click the
warning to check the employees and register their necessary information.

.. image:: hong_kong/employee-without-mpf.png
   :alt: Employee without MPF Registration
