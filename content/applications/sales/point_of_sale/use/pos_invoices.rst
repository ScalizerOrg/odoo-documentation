...
Use one of the following methods to create an invoice, depending on the workflow: :ref:`during
the ordering process <pos/invoices/generate-payment>`, :ref:`after the ordering process
<pos/invoices/generate-overview>`, or :ref:`by grouping multiple sales orders for the customer
<pos/invoices/generate-global>`.
...
#. Go to :menuselection:`Point of Sale --> Orders --> Orders`.
#. Click the search bar to filter by :guilabel:`Customer`.
#. Click the :icon:`fa-caret-right` (:guilabel:`caret`) icon next to the relevant customer and
   enable :guilabel:`Order Ref`.
#. Click :guilabel:`Create Invoices`, keep the :guilabel:`Consolidated Billing` option enabled,
   then click :guilabel:`Create`.

.. note::
   The :guilabel:`Consolidated Billing` option is enabled by default. To create an individual
   invoice for each sales order, disable the option.

.. tip::
   Alternatively, go to :menuselection:`Point of Sale --> Orders --> Customers`, select the relevant
   customer, click the :icon:`fa-shopping-bag` :guilabel:`PoS Orders` smart button on the customer
   form, select the relevant orders, click :guilabel:`Create Invoices`, then :guilabel:`Create`.
...
:guilabel:`Unique code`), and click :guilabel:`Request Invoice`. Then, they must enter
their billing information or sign in and click :guilabel:`Get my invoice`. The invoice is then
generated and available for download, and the order status is updated to :guilabel:`Fully Invoiced`.
